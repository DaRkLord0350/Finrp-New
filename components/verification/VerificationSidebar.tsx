...
import { LayoutDashboard, BadgeCheck, Landmark, ChevronRight } from "lucide-react";
import { cn } from "@/lib/utils";

type NavItem = { label: string; href: string; icon: typeof LayoutDashboard; exact?: boolean };

const verificationNav: NavItem[] = [
  { label: "Dashboard", href: "/verification", icon: LayoutDashboard, exact: true },
  { label: "Cases", href: "/verification/cases", icon: BadgeCheck },
  { label: "IFSC Lookup", href: "/verification/ifsc-lookup", icon: Landmark },
];
  ...
  return (
    ...
    >
      <div style={{ padding: "0 8px 12px", borderBottom: "1px solid var(--border)" }}>
        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--text-muted)" }}>Verification</p>
        <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 2 }}>Background Verification</p>
      </div>

      <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 1, marginTop: 12 }}>
        {verificationNav.map((item) => {
