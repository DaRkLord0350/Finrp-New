...
import { LayoutDashboard, Radar, FolderKanban, Settings2, ChevronRight } from "lucide-react";
import { cn } from "@/lib/utils";

type NavItem = { label: string; href: string; icon: typeof LayoutDashboard; exact?: boolean };

const monitoringNav: NavItem[] = [
  { label: "Dashboard", href: "/monitoring", icon: LayoutDashboard, exact: true },
  { label: "Alerts", href: "/monitoring/alerts", icon: Radar },
  { label: "Cases", href: "/monitoring/cases", icon: FolderKanban },
  { label: "Rules", href: "/monitoring/rules", icon: Settings2 },
];
  ...
  return (
    ...
    >
      <div style={{ padding: "0 8px 12px", borderBottom: "1px solid var(--border)" }}>
        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--text-muted)" }}>Monitoring</p>
        <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 2 }}>Continuous Monitoring</p>
      </div>
