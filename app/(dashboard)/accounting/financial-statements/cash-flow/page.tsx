...
import {
  RefreshCw, Plus, Download, CheckCircle2,
  Send, Lock, ThumbsUp, Sparkles, SlidersHorizontal,
...
function StatusBadge({ status }: { status: ReportStatus }) {
  ...
  return <span style={{ fontSize: 11, fontWeight: 700, color: c.fg, background: c.bg, padding: "3px 10px", borderRadius: 6 }}>{status}</span>;
}
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 13, fontWeight: row.bold ? 700 : 400, fontStyle: row.italic ? "italic" : undefined, color: row.bold ? "var(--text-primary)" : "var(--text-secondary)", display: "flex", alignItems: "center", gap: 6 }}>
        {row.label}
        {row.noteRef !== undefined && <sup style={{ fontSize: 10, color: "#6366f1", fontWeight: 600 }}>[{row.noteRef}]</sup>}
      </div>
      <div style={{ textAlign: "right", fontSize: 13, fontWeight: row.bold ? 700 : 400, fontStyle: row.italic ? "italic" : undefined, color: row.bold ? "var(--text-primary)" : "var(--text-secondary)" }}>
        {row.currentAmount >= 0 ? "" : "("}{formatCurrency(Math.abs(row.currentAmount))}{row.currentAmount < 0 ? ")" : ""}
      </div>
      {comparative && (
...
function WaterfallBar({ label, value, total, color }: { label: string; value: number; total: number; color: string }) {
  const pct = total === 0 ? 0 : Math.abs(value / total) * 100;
  const isPositive = value >= 0;
  return (
    <div style={{ flex: 1, padding: "0 6px" }}>
      <p style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6, textAlign: "center" }}>{label}</p>
      <div style={{ height: 80, background: "var(--bg-elevated)", borderRadius: 8, position: "relative", overflow: "hidden", marginBottom: 6 }}>
        <div style={{
          position: "absolute",
          ...
          background: color,
          opacity: 0.75,
          borderRadius: isPositive ? "0 0 8px 8px" : "8px 8px 0 0",
      ...
      <p style={{ fontSize: 12, fontWeight: 700, color, textAlign: "center" }}>
...
export default function CashFlowPage() {
  ...
  const loadReports = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/financial-statements/reports?type=CASH_FLOW");
  ...
  const doAction = async (action: string) => {
    ...
    setError(null);
    try {
      const statusMap: Record<string, string> = { review: "REVIEW", approve: "APPROVED", lock: "LOCKED" };
      if (action === "validate") {
  ...
  // Waterfall total = max absolute value for scaling
  const waterfallTotal = kpis
    ? Math.max(Math.abs(kpis.operatingCF), Math.abs(kpis.investingCF), Math.abs(kpis.financingCF), Math.abs(kpis.netChange), 1)
    : 1;
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "var(--text-primary)", letterSpacing: "-0.02em" }}>Cash Flow Statement</h1>
          <p style={{ color: "var(--text-secondary)", fontSize: 14, marginTop: 3 }}>Indirect Method — Operating, Investing &amp; Financing Activities</p>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          {!loadingReports && (
            <select value={selectedId ?? ""} onChange={(e) => setSelectedId(e.target.value)}
              ...
              {reports.map((r) => <option key={r.id} value={r.id}>{r.name || `CF ${new Date(r.periodStart).getFullYear()}`}</option>)}
            </select>
          )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
        {[
          { label: "Operating CF",  value: kpis?.operatingCF, color: CF_COLORS.operating },
          { label: "Investing CF",  value: kpis?.investingCF, color: CF_COLORS.investing },
          { label: "Financing CF",  value: kpis?.financingCF, color: CF_COLORS.financing },
          { label: "Net Change",    value: kpis?.netChange,   color: CF_COLORS.net },
        ].map((s) => (
          <div key={s.label} className="stat-card" style={{ padding: "18px 20px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <Droplets size={14} color={s.color} />
            </div>
            <p style={{ fontSize: 18, fontWeight: 700, color: (s.value ?? 0) >= 0 ? s.color : "#ef4444", marginTop: 6 }}>
              {loadingSnapshot ? "…" : (s.value !== undefined ? formatCurrency(s.value) : "—")}
            </p>
          </div>
        ))}
      </div>
      ...
      {kpis && !loadingSnapshot && (
        <div className="surface" style={{ padding: "18px 20px", marginBottom: 16 }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 14 }}>Cash Flow Waterfall</p>
          <div style={{ display: "flex", gap: 4, alignItems: "flex-end" }}>
            <WaterfallBar label="A: Operating" value={kpis.operatingCF} total={waterfallTotal} color={CF_COLORS.operating} />
            <div style={{ width: 2, height: 80, background: "var(--border)", alignSelf: "center", flexShrink: 0 }} />
            <WaterfallBar label="B: Investing"  value={kpis.investingCF}  total={waterfallTotal} color={CF_COLORS.investing} />
            <div style={{ width: 2, height: 80, background: "var(--border)", alignSelf: "center", flexShrink: 0 }} />
            <WaterfallBar label="C: Financing" value={kpis.financingCF} total={waterfallTotal} color={CF_COLORS.financing} />
            <div style={{ width: 2, height: 80, background: "var(--border)", alignSelf: "center", flexShrink: 0 }} />
            <WaterfallBar label="Net Change"   value={kpis.netChange}   total={waterfallTotal} color={CF_COLORS.net} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 14, padding: "10px 14px", background: "var(--bg-elevated)", borderRadius: 8 }}>
            <div>
              <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Opening Cash: </span>
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{formatCurrency(kpis.openingCash)}</span>
            </div>
            <div>
              <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Net Change: </span>
              <span style={{ fontSize: 13, fontWeight: 600, color: kpis.netChange >= 0 ? "#10b981" : "#ef4444" }}>{kpis.netChange >= 0 ? "+" : ""}{formatCurrency(kpis.netChange)}</span>
            </div>
            <div>
              <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Closing Cash: </span>
              <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>{formatCurrency(kpis.closingCash)}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {loadingSnapshot ? (
        <div className="surface" style={{ padding: 60, textAlign: "center" }}>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: 13, marginTop: 12 }}>Computing cash flow statement…</p>
        </div>
      ) : !selectedId ? (
        <div className="surface" style={{ padding: 60, textAlign: "center" }}>
          <p style={{ color: "var(--text-muted)", fontSize: 14 }}>Select a report above or create a new Cash Flow report.</p>
        </div>
      ) : sections.length === 0 ? (
        <div className="surface" style={{ padding: 60, textAlign: "center" }}>
        ...
        <motion.div className="surface" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ overflow: "hidden", marginBottom: 72 }}>
          <div style={{ display: "grid", gridTemplateColumns: `1fr 180px${comparativeMode ? " 180px" : ""}`, padding: "10px 20px", borderBottom: "2px solid var(--border-strong)", background: "var(--bg-elevated)" }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em", textAlign: "right" }}>
              {report ? `FY ${new Date(report.periodEnd).getFullYear()}` : "Current Year"}
            </div>
            {comparativeMode && (
              <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em", textAlign: "right" }}>Prior Year</div>
            )}
          </div>

          {sections.map((section, si) => {
            const sectionColors = [CF_COLORS.operating, CF_COLORS.investing, CF_COLORS.financing];
            const sectionColor = sectionColors[si] ?? "#9ca3af";
            return (
              <div key={section.key}>
                <div style={{ padding: "12px 20px 4px", background: `${sectionColor}08`, borderTop: "1px solid var(--border)" }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: sectionColor, textTransform: "uppercase", letterSpacing: "0.04em" }}>{section.title}</span>
                </div>
                {section.rows.map((row, i) => <ReportRow key={i} row={row} comparative={comparativeMode} />)}
                {section.subtotal && <ReportRow row={{ ...section.subtotal, italic: true }} comparative={comparativeMode} />}
                {section.total && <ReportRow row={{ ...section.total, bold: true, borderTop: true }} comparative={comparativeMode} />}
              </div>
            );
          })}
        </motion.div>
