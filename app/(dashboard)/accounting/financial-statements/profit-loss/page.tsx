...
import {
  ...
  Send, Lock, ThumbsUp, Sparkles, SlidersHorizontal, TrendingUp, TrendingDown,
} from "lucide-react";
...
export default function ProfitLossPage() {
  ...
  const loadReports = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/financial-statements/reports?type=PROFIT_LOSS");
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "var(--text-primary)", letterSpacing: "-0.02em" }}>Profit & Loss Statement</h1>
          <p style={{ color: "var(--text-secondary)", fontSize: 14, marginTop: 3 }}>Statement of Profit and Loss — Schedule III</p>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          {!loadingReports && (
            <select value={selectedId ?? ""} onChange={(e) => setSelectedId(e.target.value)}
              ...
              {reports.map((r) => <option key={r.id} value={r.id}>{r.name || `P&L ${new Date(r.periodStart).getFullYear()}`}</option>)}
            </select>
          )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
        {[
          { label: "Total Revenue",    value: kpis ? formatCurrency(kpis.totalRevenue)   : "—", color: "#10b981", icon: TrendingUp },
          { label: "Total Expenses",   value: kpis ? formatCurrency(kpis.totalExpenses)  : "—", color: "#ef4444", icon: TrendingDown },
          { label: "Net Profit",       value: kpis ? formatCurrency(kpis.netProfit)      : "—", color: profitPositive ? "#10b981" : "#ef4444", icon: profitPositive ? TrendingUp : TrendingDown },
          { label: "Profit Margin",    value: kpis ? `${kpis.profitMargin.toFixed(1)}%`  : "—", color: profitPositive ? "#6366f1" : "#f59e0b", icon: TrendingUp },
        ].map((s) => {
          const Icon = s.icon;
          return (
            <div key={s.label} className="stat-card" style={{ padding: "18px 20px" }}>
              ...
              <p style={{ fontSize: 20, fontWeight: 700, color: s.color, marginTop: 6 }}>{loadingSnapshot ? "…" : s.value}</p>
            </div>
          );
        })}
      </div>
      ...
      {loadingSnapshot ? (
        <div className="surface" style={{ padding: 60, textAlign: "center" }}>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: 13, marginTop: 12 }}>Computing P&L statement…</p>
        </div>
      ) : !selectedId ? (
        <div className="surface" style={{ padding: 60, textAlign: "center" }}>
          <p style={{ color: "var(--text-muted)", fontSize: 14 }}>Select a report above or create a new P&L report.</p>
        </div>
      ) : sections.length === 0 ? (
        <div className="surface" style={{ padding: 60, textAlign: "center" }}>
        ...
        <motion.div className="surface" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ overflow: "hidden", marginBottom: 72 }}>
          <div style={{ display: "grid", gridTemplateColumns: `1fr 180px${comparativeMode ? " 180px" : ""}`, padding: "10px 20px", borderBottom: "2px solid var(--border-strong)", background: "var(--bg-elevated)" }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em", textAlign: "right" }}>
              {report ? `${new Date(report.periodStart).getFullYear()}-${String(new Date(report.periodEnd).getFullYear()).slice(-2)}` : "Current Year"}
            </div>
            {comparativeMode && (
          ...
          {sections.map((section) => (
            <div key={section.key}>
              <div style={{ padding: "12px 20px 4px", background: "rgba(16,185,129,0.04)", borderTop: "1px solid var(--border)" }}>
