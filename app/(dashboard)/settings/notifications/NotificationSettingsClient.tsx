...
export default function NotificationSettingsClient({ initialConfig }: { initialConfig: NotificationConfig }) {
  const [saving, setSaving] = useState(false);

  const [orgEmail, setOrgEmail] = useState(initialConfig.org?.emailNotifications ?? true);
  const [orgSms, setOrgSms] = useState(initialConfig.org?.smsNotifications ?? false);
  const [overdueReminderDays, setOverdueReminderDays] = useState(initialConfig.org?.overdueReminderDays ?? 3);
  const [lowStockThreshold, setLowStockThreshold] = useState(initialConfig.org?.lowStockThreshold ?? 10);
  const [userPrefs, setUserPrefs] = useState<Record<string, boolean>>(() => {
    const prefs: Record<string, boolean> = {};
    NOTIFICATION_TYPES.forEach((t) => { prefs[t.key] = initialConfig.user?.[t.key] !== false; });
    return prefs;
  });
