...
const STATUS_OPTIONS = ["All", "OPEN", "IN_PROGRESS", "ON_HOLD", "COMPLETED", "REJECTED"];

export default function VerificationCasesPage() {
  const [status, setStatus] = useState("All");
  const { data, isLoading } = useQuery<{ cases: VerificationCaseRow[] }>(["verification", "cases", status], () =>
    api(`/api/verification/cases${status !== "All" ? `?status=${status}` : ""}`)
  );

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700 }}>Verification Cases</h1>
        ...
        <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
          ...
          <tbody>
            ...
            {!isLoading && (data?.cases?.length ?? 0) === 0 && (
              <tr><td colSpan={7} style={{ padding: 24, textAlign: "center", color: "var(--text-muted)" }}>No cases found.</td></tr>
            )}
            {data?.cases?.map((c) => (
