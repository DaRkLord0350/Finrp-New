...
export default function AMLCaseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const qc = useQueryClient();
  const { data, isLoading } = useQuery<{ case: CaseDetail }>(["aml", "case", id], () => api(`/api/aml/cases/${id}`));
  const refresh = () => qc.invalidate(["aml", "case", id]);
  ...
  const escalate = async () => {
    try { await api(`/api/aml/cases/${id}`, { method: "PATCH", body: JSON.stringify({ action: "escalate", notes: note }) }); toast.success("Case escalated"); refresh(); }
    catch (e) { toast.error((e as Error).message); }
  };
  const addNote = async () => {
    if (!note) return;
    try { await api(`/api/aml/cases/${id}/notes`, { method: "POST", body: JSON.stringify({ notes: note }) }); setNote(""); toast.success("Note added"); refresh(); }
    catch (e) { toast.error((e as Error).message); }
  };
  const resolve = async (resolution: "CLEARED" | "FALSE_POSITIVE" | "CONFIRMED_SAR") => {
    if (!resolutionNotes) return toast.error("Resolution notes are required");
    try {
      await api(`/api/aml/cases/${id}`, { method: "PATCH", body: JSON.stringify({ action: "resolve", resolution, resolutionNotes }) });
      toast.success(`Case resolved: ${resolution}`);
      refresh();
    } catch (e) { toast.error((e as Error).message); }
  };
  const summarize = async () => {
    try { const res = await api(`/api/aml/cases/${id}/summarize`, { method: "POST" }); setSummary(res.summary); }
    catch (e) { toast.error((e as Error).message); }
  };

  if (isLoading || !data) return <p style={{ color: "var(--text-muted)" }}>Loading case…</p>;
  const amlCase = data.case;
  const isOpen = ["OPEN", "UNDER_REVIEW", "ESCALATED"].includes(amlCase.status);

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700 }}>{amlCase.caseNumber}</h1>
          <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4 }}>{amlCase.subjectName}</p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <StatusBadge status={amlCase.riskRating} size="md" />
          <StatusBadge status={amlCase.status} size="md" />
        </div>
      </div>
      ...
      {isOpen ? (
        <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 14, padding: 18 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={() => resolve("CONFIRMED_SAR")} style={dangerBtn}>File SAR</button>
          </div>
        </div>
      ) : (
        <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 14, padding: 18 }}>
          <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 6 }}>Resolution</h3>
          <p style={{ fontSize: 13 }}>{amlCase.resolution} — {amlCase.resolutionNotes}</p>
        </div>
      )}
    </div>
  );
