...
const STATUS_OPTIONS = ["All", "OPEN", "UNDER_REVIEW", "ESCALATED", "CLEARED", "CONFIRMED_FRAUD", "CLOSED"];

export default function FraudCasesPage() {
  const [status, setStatus] = useState("All");
  const { data, isLoading } = useQuery<{ cases: FraudCaseRow[] }>(["fraud", "cases", status], () =>
    api(`/api/fraud/cases${status !== "All" ? `?status=${status}` : ""}`)
  );

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700 }}>Fraud Cases</h1>
      ...
      <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
        <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
          ...
          <tbody>
            ...
            {!isLoading && (data?.cases?.length ?? 0) === 0 && <tr><td colSpan={6} style={{ padding: 24, textAlign: "center", color: "var(--text-muted)" }}>No cases found.</td></tr>}
            {data?.cases?.map((c) => (
