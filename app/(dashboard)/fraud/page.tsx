...
export default function FraudDashboardPage() {
  const { data, isLoading } = useQuery<{ cases: FraudCaseRow[] }>(["fraud", "cases-all"], () => api("/api/fraud/cases"));
  ...
  const critical = cases.filter((c) => c.riskRating === "CRITICAL");

  return (
    <div>
      <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>Fraud Dashboard</h1>
      <p style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 20 }}>Duplicate identity, velocity, blacklist, and rules-engine fraud scoring</p>
      ...
      {isLoading ? (
        ...
        <>
          <StatGrid
            columns={3}
            stats={[
              { label: "Open Cases", value: open.length, icon: <ShieldX size={20} />, color: "#f59e0b" },
              { label: "Critical Risk", value: critical.length, icon: <ShieldX size={20} />, color: "#ef4444" },
              { label: "Confirmed Fraud", value: confirmed.length, icon: <ShieldX size={20} />, color: "#7f1d1d" },
            ]}
          />
          ...
          <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
            <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
              ...
              <tbody>
                {cases.length === 0 && <tr><td colSpan={6} style={{ padding: 24, textAlign: "center", color: "var(--text-muted)" }}>No fraud cases yet.</td></tr>}
                {cases.map((c) => (
                  <tr key={c.id} style={{ borderTop: "1px solid var(--border)" }}>
                    <td style={{ padding: "12px 16px" }}>
                      <Link href={`/fraud/cases/${c.id}`} style={{ color: "#818cf8", fontWeight: 600, textDecoration: "none" }}>{c.caseNumber}</Link>
                    </td>
