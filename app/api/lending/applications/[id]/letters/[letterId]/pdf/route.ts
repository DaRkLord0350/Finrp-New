...
import { renderLoanLetterPdfBuffer } from "@/lib/pdf/generateLoanLetterPdf";

export async function GET(req: Request, { params }: { params: Promise<{ id: string; letterId: string }> }) {
  try {
    ...
    const { buffer, pdfFileName } = await renderLoanLetterPdfBuffer(letterId, organizationId);
    ...
    console.error("[LENDING_LETTER_PDF]", error);
